...
(
    ...
    and
    (
        label_time = date('now', '-28 day', 'weekday ${weekday}')
        or
        label_time = date('now', '-35 day', 'weekday ${weekday}')
    )
)t1
...
(
    ...
    and
    (
        label_time = date('now', '-2${weekday} day', 'weekday ${weekday}')
        or
        label_time = date('now', '-${weekday}4 day', 'weekday ${weekday}')
        or
        label_time = date('now', '-7 day', 'weekday ${weekday}')
    )
)t1
group by code, label_time, label_rate
;
